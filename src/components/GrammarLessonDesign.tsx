...
/* ─────────────────────────────────────────────────────────────────────────
   SAFE HTML RENDERER
   🔒 FIX (audit #16): this component was named "SafeHtml" but didn't
   actually sanitize anything — it only added a CSS class to <strong> tags.
   Today's actual risk is low (lesson content comes from developer-authored
   files under src/content/, not user input or a database), but adding real
   sanitization here is cheap, correct defense-in-depth, and means this
   component's name stops being aspirational. Explicit allowlist (rather
   than DOMPurify's broader defaults) because that's the exact, complete set
   of tags this function and its callers ever produce.
───────────────────────────────────────────────────────────────────────── */
const SAFE_HTML_ALLOWED_TAGS = ["strong", "em", "code", "span", "p", "li", "ul", "ol", "br"];
const SAFE_HTML_ALLOWED_ATTR = ["class"];
                ...
                <td
                  ...
                  dangerouslySetInnerHTML={{
                    __html: DOMPurify.sanitize(
                      ...
                      { ALLOWED_TAGS: SAFE_HTML_ALLOWED_TAGS, ALLOWED_ATTR: SAFE_HTML_ALLOWED_ATTR }
                    ),
                  }}
                />
              ))}
