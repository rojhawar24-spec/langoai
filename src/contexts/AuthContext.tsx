  ...
  const updateProfile = useCallback(
    async (updates: Partial<UserData>) => {
      ...
      try {
        ...
        if (!session?.user) {
          // No session: do NOT invent success for any update
          console.error("updateProfile: no session — aborting");
          return;
        }
        ...
        if (touchesGameState) {
          ...
          // XP: never mint by client-chosen amount. Map known deltas to
          // activity kinds; server decides the XP in complete_learning_activity.
          const amount =
            updates.totalXP !== undefined ? updates.totalXP - user.totalXP : 0;

          if (amount > 0) {
            const { activityKindFromAmount } = await import("@/utils/xp");
            const kind = activityKindFromAmount(amount);
            if (!kind) {
              console.error("updateProfile: unknown XP amount — rejected", amount);
              return;
            }
            ...
            if (error) {
              console.error("complete_learning_activity failed — no local invent:", error);
              return;
            }
            const profile = (data as { profile?: unknown })?.profile ?? data;
            if (profile && typeof profile === "object") {
              const updated = rowToUser(profile as import("@/lib/supabase").ProfileRow);
              setCurrentUser(updated);
              setUser(updated);
            }
          }
        ...
        // FAIL: never invent success state after an error
      }
    },
    [user]
  );
