...
function SeasonView({ season, lang, onBack, award, themeConfig, premium = false }: {
  ...
  const finishLvl = (score: number, max: number) => {
    ...
    if (passed) {
      ...
      award(XP_PER_LEVEL);
      // 🔒 was: setCoins(addCoins(COINS_PER_LEVEL)) — a pure localStorage
      // write, no different from editing your own save file. Fire-and-
      // forget (same pattern as XP in xp.ts): the level-complete popup
      // below shouldn't wait on a network round trip to appear.
      supabase.rpc("earn_coins", { p_amount: COINS_PER_LEVEL }).then(({ data, error }) => {
        if (!error && data) {
          setCoins(data.coins);
          refreshUser();
        } else {
          console.error("earn_coins RPC failed \u2014 coins not persisted server-side:", error);
        }
      });
