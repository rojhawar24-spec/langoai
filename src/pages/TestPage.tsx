...
import { getTestById, getTestList } from "@/content/tests/lazy";
...
function TestTaker({
  ...
  const score = test.questions.reduce((acc, question, i) => (answers[i] === question.answer ? acc + 1 : acc), 0);
  ...
  const handleSubmit = () => {
    ...
    if (passed) {
      const reward = awardRef.current(XP_REWARDS.TEST_PASSED);
      if (reward.leveledUp) {
    ...
    test.questions.forEach((question, i) => {
      if (answers[i] && answers[i] !== question.answer) {
        addMistake({
        ...
        <div className="mx-auto max-w-2xl px-4 py-12 text-center sm:px-6">
          ...
          <div className="grid grid-cols-3 gap-4 mt-8">
            <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
              ...
              <p className="text-xs text-slate-500">{t("testpage.score")}</p>
            </div>
            ...
            <div className={`rounded-xl border p-4 ${passed ? "border-emerald-200 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-900/20" : "border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20"}`}>
              ...
              <p className="text-xs text-slate-500">{t("testpage.passRequirement").replace("{score}", String(test.passScore))}</p>
            </div>

            <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
              <p className="text-3xl font-bold text-indigo-600">{passed ? `+${XP_REWARDS.TEST_PASSED}` : "0"}</p>
              <p className="text-xs text-slate-500">{t("topbar.xp")}</p>
            </div>
          </div>

          <div className="flex flex-col sm:flex-row gap-3 justify-center mt-8">
            <button type="button" onClick={onBack} className="rounded-xl bg-indigo-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-indigo-700">
              {t("tests.back")}
            </button>
            <button type="button" onClick={onDone} className="rounded-xl border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
              {t("testpage.dashboard")}
            </button>
          </div>
        </div>
      </div>
  ...
  if (!q) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <button type="button" onClick={onBack} className="rounded-xl bg-indigo-600 px-6 py-3 text-sm font-semibold text-white">
          {t("tests.back")}
        </button>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        <div className="flex items-center justify-between mb-6">
          <button type="button" onClick={onBack} className="text-sm text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400">
            {t("tests.back")}
          </button>
          <span className="text-xs text-slate-400">{current + 1}/{max}</span>
          <button type="button" onClick={() => setSubmitted(true)} className="text-sm font-semibold text-indigo-500">
            {t("tests.submit")}
          </button>
        </div>

        <div className="h-1.5 rounded-full bg-slate-200 dark:bg-slate-700 mb-8">
          <div className="h-full rounded-full bg-indigo-500 transition-all" style={{ width: `${((current + 1) / max) * 100}%` }} />
        </div>

        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1 text-xs font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-300 mb-4">
            {test.topic}
          </div>
            ...
            {q.options.map((opt) => {
              ...
              return (
                <button type="button" key={opt}
                  ...
                  className={`w-full rounded-xl border-2 px-5 py-3.5 text-left text-sm font-medium transition ${
                    chosen
                ...
                >
                  <span className="flex items-center gap-3">
                    <span className={`flex h-6 w-6 items-center justify-center rounded-full text-xs ${chosen ? "bg-indigo-500 text-white" : "border border-slate-300 text-slate-400 dark:border-slate-500"}`}>
                      {chosen ? "✓" : "○"}
                    </span>
                    {opt}
                  </span>
          ...
          <div className="flex justify-between mt-6">
            <button type="button" onClick={() => setCurrent(Math.max(0, current - 1))}
              disabled={current === 0}
              className="rounded-lg border px-4 py-2 text-sm text-slate-600 disabled:opacity-30 dark:border-slate-700 dark:text-slate-400"
            >
              {t("tests.previous")}
            </button>

            {current + 1 < max ? (
              <button type="button" onClick={() => setCurrent(current + 1)} className="rounded-lg bg-indigo-600 px-5 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700">
                {t("tests.next")}
              </button>
            ) : (
              <button type="button" onClick={handleSubmit}
                disabled={answers.filter(Boolean).length < max * 0.5}
...
export default function TestPage() {
  ...
  const tests = search
    ? allTests.filter((test) => test.title.toLowerCase().includes(search.toLowerCase()) || test.topic.toLowerCase().includes(search.toLowerCase()))
    : allTests;
  ...
  const openTest = async (test: TestMeta) => {
    setLoading(true);
    const fullTest = await getTestById(lang, test.id);
    setLoading(false);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-slate-600 dark:text-slate-400">{t("testpage.loadingTests")}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#f8f6f2] dark:bg-[#14141e]">
      <div className="sticky top-0 z-30 border-b border-slate-200/70 bg-white/85 backdrop-blur-lg dark:border-slate-700/70 dark:bg-slate-900/85">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <button type="button" onClick={() => nav("/dashboard")} className="text-sm text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400">
            {t("testpage.back")}
          </button>
          <h1 className="text-sm font-bold text-slate-800 dark:text-white sm:text-base">
            {t("testpage.testsTitle").replace("{flag}", LANGUAGE_FLAGS[lang])}
          </h1>
          <span className="text-xs text-slate-500">{allTests.length} {t("testpage.testsCount")}</span>
        </div>
      ...
      <div className="mx-auto mt-5 max-w-lg px-4">
          ...
          {LEVELS.map((lvl, i) => (
            <button type="button" key={lvl}
              ...
              className={`flex-1 rounded-xl py-2.5 text-sm font-black tracking-wide transition-all duration-200 sm:text-base sm:py-3 ${
                search
                  ? "bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-600 opacity-50"
                  : activeLevel === lvl
        ...
        {search ? (
          <p className="mt-2 text-center text-[10px] text-indigo-400">{t("testpage.levelFocus")}</p>
        ) : (
      ...
      <div className="mx-auto mt-6 max-w-2xl px-4 pb-24">
        <div className={`mb-4 flex items-center gap-2 rounded-xl ${search ? "bg-indigo-50 border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-700" : cls.light} border px-4 py-2.5`}>
          <span className="font-black tracking-wide text-slate-700 dark:text-slate-200">
            {search ? t("tests.allLevels") : activeLevel}
          </span>

          {search && (
            <button type="button" onClick={() => setSearch("")} className="ml-2 text-xs text-indigo-500 hover:text-indigo-600">
              {t("testpage.clear")}
            </button>
          )}

          <span className="ml-auto text-xs text-slate-500">
            {activeTests.length} {search ? t("testpage.results") : t("testpage.testsCount")}
          </span>
        </div>

        {activeTests.length === 0 ? (
          <div className={`rounded-xl border-2 border-dashed p-8 text-center ${cls.light}`}>
            <p className="text-sm text-slate-500">{t("testpage.noTestsFound")}</p>
          </div>
        ) : (
          <div className="space-y-3">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[10px] text-slate-400">{test.questionCount} {t("testpage.questions")}</span>
                      <span className="text-[10px] text-slate-400">{t("testpage.passRequirement").replace("{score}", String(test.passScore))}</span>
                    </div>

                    <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">{test.title}</h3>
                    <p className="text-xs text-indigo-500 mt-1">+{XP_REWARDS.TEST_PASSED} {t("topbar.xp")}</p>
                  </div>
