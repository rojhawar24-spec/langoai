...
// ✅ DIRECTE IMPORT – geen dynamic import meer
import { nlGrammar } from "@/content/grammar/nl/index";
...
export default function GrammarPage() {
  ...
  const [allLessons] = useState<GrammarLesson[]>(nlGrammar);
  ...
  const filteredLessons = search ? lessons : lessons.filter(l => lvlToCEFR(l.level) === activeLevel);
  ...
  const snapTo = (i: number) => { setSearch(""); setActiveLevel(LEVELS[i]); };
        ...
        <div className="flex items-center justify-between gap-1 sm:gap-2">
          {LEVELS.map((lvl, i) => (
            <button key={lvl} onClick={() => snapTo(i)}
              className={`flex-1 rounded-xl py-2.5 text-sm font-black tracking-wide transition-all duration-200 sm:text-base sm:py-3 ${activeLevel === lvl ? `${lvlColors[lvl].bg} text-white shadow-lg` : "bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-600"}`}
            >
              {lvl}
            </button>
          ))}
              ...
              {visibleLessons.map((lesson) => {
                ...
                return (
                  <button key={lesson.id} onClick={() => openLesson(lesson)}
                    className={`w-full rounded-xl border-2 p-4 text-left shadow-sm transition-all duration-200 hover:shadow-md active:scale-[0.99] ${done ? "border-emerald-300 bg-emerald-50/60 dark:border-emerald-600 dark:bg-emerald-900/10" : "border-slate-200 bg-white hover:border-indigo-300 dark:border-slate-700 dark:bg-slate-800 dark:hover:border-indigo-500"}`}
                  >
