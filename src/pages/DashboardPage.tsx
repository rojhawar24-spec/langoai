...
export default function DashboardPage() {
  ...
  async function claimChest() {
    ...
    // 🔒 was: addCoins(DAILY_CHEST_REWARD) — a pure localStorage write.
    const { data, error } = await supabase.rpc("earn_coins", { p_amount: DAILY_CHEST_REWARD });
    if (error || !data) {
      console.error("earn_coins RPC failed (daily chest):", error);
      return;
    }
    try {
      localStorage.setItem(DAILY_CHEST_KEY, new Date().toISOString().slice(0, 10));
    } catch {
      // localStorage kan falen — mag het claimen zelf niet blokkeren
    }
