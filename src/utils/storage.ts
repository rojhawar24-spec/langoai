...
export function userToRow(updates: Partial<UserData>): Partial<ProfileRow> {
  ...
  if (updates.lastActivityDate !== undefined) row.last_activity_date = updates.lastActivityDate;
  ...
  if (updates.premiumExpiresAt !== undefined) row.premium_expires_at = updates.premiumExpiresAt;
  return row;
}
...
export async function apiLogin(
  ...
  if (aal && aal.nextLevel === "aal2" && aal.currentLevel !== "aal2") {
    ...
    if (!totpFactor) {
      // Zou niet moeten gebeuren (nextLevel=aal2 impliceert een factor),
      // maar fail-safe: niet inloggen zonder duidelijke oorzaak.
      throw new Error("mfa_error");
    }
    // Wachtwoord was correct → teller resetten, maar login is nog niet compleet
