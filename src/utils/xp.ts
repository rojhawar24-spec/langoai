...
import { type UserData, getCurrentUser, rowToUser, setCurrentUser } from "@/utils/storage";
...
// ── Server-authoritative XP (CLIENT cannot choose amount) ────────────────
...
export function activityKindFromAmount(amount: number): LearningActivityKind | null {
  switch (amount) {
    case XP_REWARDS.GRAMMAR_LESSON_COMPLETE: return "grammar_lesson";
    case XP_REWARDS.VOCABULARY_WORD_LEARNED: return "vocabulary_word";
    case XP_REWARDS.EXERCISE_CORRECT: return "exercise_correct";
    case XP_REWARDS.TEST_PASSED: return "test_passed";
    case XP_REWARDS.DAILY_GOAL_BONUS: return "daily_goal";
    case XP_REWARDS.STREAK_7_DAYS: return "streak_7";
    case XP_REWARDS.STREAK_30_DAYS: return "streak_30";
    default: return null;
  }
}

/** Server decides XP. Returns updated user or null on failure (no fake success). */
export async function completeLearningActivity(
  ...
  if (error) {
    console.error("complete_learning_activity failed:", error.message);
    return null;
  }
  const profile = (data as { profile?: unknown })?.profile ?? data;
  if (!profile || typeof profile !== "object") return null;
  const user = rowToUser(profile as import("@/lib/supabase").ProfileRow);
  setCurrentUser(user);
  return user;
}
