...
export function isPremiumActive(
  ...
  // ✅ FIX (audit, gevonden tijdens het schrijven van tests): hieronder stond
  // voorheen `user?.premiumExpiresAt ?? getPaymentExpiresAt()`. Omdat ?? ook
  // bij expliciet `null` doorvalt naar rechts, viel dit voor een heel gewone
  // niet-premium gebruiker (premiumExpiresAt: null is de standaardwaarde in
  // de database) altijd terug op localStorage — precies het lek dat we in
  // PremiumPage.tsx dachten te hebben dichtgezet. Nu: zodra er een user-
  // object is meegegeven, is dát de enige bron van waarheid, ook als de
  // waarde null is. Alleen zonder user-object (bv. uitgelogd) valt dit terug
  // op localStorage.
  const expiresAt = user ? user.premiumExpiresAt : getPaymentExpiresAt();
