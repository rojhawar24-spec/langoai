...
-- ── 5. grant_premium_entitlement (service_role / webhook only) ─
-- Not granted to authenticated/anon. Called via service role from API.

CREATE OR REPLACE FUNCTION public.grant_premium_entitlement(
  p_user_id UUID,
  p_days INTEGER DEFAULT 30,
  p_provider TEXT DEFAULT 'kofi',
...
BEGIN
  ...
  IF p_transaction_id IS NOT NULL AND length(trim(p_transaction_id)) > 0 THEN
    BEGIN
      INSERT INTO public.processed_payments (
        provider, transaction_id, user_id, email, amount, currency, premium_days
      ) VALUES (
        COALESCE(NULLIF(trim(p_provider), ''), 'kofi'),
    ...
    EXCEPTION
      WHEN unique_violation THEN
        -- Idempotent: already processed this transaction
  ...
  v_base := CASE
    WHEN v_profile.premium IS TRUE
         AND v_profile.premium_expires_at IS NOT NULL
         AND v_profile.premium_expires_at > NOW()
    THEN v_profile.premium_expires_at
    ELSE NOW()
  END;

  UPDATE public.profiles
  SET premium = TRUE,
      premium_expires_at = v_base + (v_days || ' days')::INTERVAL
  WHERE id = p_user_id
  RETURNING * INTO v_profile;
...
REVOKE EXECUTE ON FUNCTION public.grant_premium_entitlement(UUID, INTEGER, TEXT, TEXT, TEXT, NUMERIC, TEXT)
  FROM PUBLIC, authenticated, anon;
-- Only service_role (bypasses grants) will call this from the webhook API.

-- ── 6. Ensure handle_new_user creates profiles (authoritative) ─
