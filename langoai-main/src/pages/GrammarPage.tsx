      ...
      <div className="relative mx-auto mt-8 max-w-3xl px-4 pb-32">
        {!search && (
          <div className={`mb-8 flex items-center justify-between rounded-2xl border px-5 py-4 backdrop-blur-sm ${cls.light} ${cls.border}`}>
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-base font-bold text-slate-800 dark:text-slate-100">{t("grammarpage.levelHeading").replace("{level}", activeLevel)}</p>
                <p className="text-sm text-slate-500 dark:text-slate-400">{t("grammarpage.lessonsAvailable").replace("{count}", String(filteredLessons.length))}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {filteredLessons.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-200 bg-white/40 py-20 text-center dark:border-slate-800 dark:bg-slate-900/40 backdrop-blur-sm">
            ...
            <p className="text-lg font-bold text-slate-700 dark:text-slate-200">{t("grammar.noLessonsFound")}</p>
            <p className="mt-1 text-sm text-slate-500">{t("grammar.tryDifferentSearch")}</p>
          </div>
        ) : (
          <>
            ...
            {visibleCount < filteredLessons.length && (
              <button type="button" onClick={() => setVisibleCount((v) => v + 10)}
