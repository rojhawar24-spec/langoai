...
// groeien vanzelf terug aan (1 per 30 min).
...
// ─── QUIZ COMPONENT ───────────────────────────────────────────────────────────
function Quiz({ questions, onDone, themeColor, lang = "en", onOpenShop }: {
  questions: Question[];
  ...
  const handleNewTypeAnswer = useCallback((correct: boolean) => {
    ...
    else { setJustLostHeart(true); setHearts(loseHeart()); setTimeout(() => setJustLostHeart(false), 500); }
  }, []);
  ...
  const handleCheck = useCallback(() => {
    ...
    if (good) { setScore(v => v + 1); soundEngine.playCorrect(); }
      ...
      { setJustLostHeart(true); setHearts(loseHeart()); setTimeout(() => setJustLostHeart(false), 500); }
    }
  }, [ok, q, inp, chosen]);
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="flex items-center gap-3">
            <HeartsDisplay hearts={hearts} dark={dark} justLost={justLostHeart} />
    ...
    return (
      <div className={`min-h-screen ${dark ? "bg-slate-900" : "bg-slate-50"}`}>
        <div className="mx-auto max-w-lg px-4 py-6">
          ...
          {lvl && <Quiz questions={lvl.questions} onDone={finishLvl} themeColor={themeConfig} lang={lang} onOpenShop={() => setShopOpen(true)} />}
        </div>
        {popup && (
