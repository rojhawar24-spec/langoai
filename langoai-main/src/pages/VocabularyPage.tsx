...
export default function VocabularyPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-slate-600 dark:text-slate-400">{t("vocabpage.loadingWords")}</p>
        </div>
      </div>
    );
  }
  ...
  if (selectedWord) {
    return (
      ...
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        ...
        <button type="button" onClick={() => setSelectedWord(null)} className="mb-6 flex items-center gap-1 text-sm text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          ...
          <div className="mt-6 rounded-xl bg-slate-50 p-5 dark:bg-slate-800/50">
            <p className="text-xs font-medium uppercase tracking-wide text-slate-400">{t("vocabpage.example")}</p>
          ...
          <div className="mt-6 flex items-center justify-between">
            {learned.has(selectedWord.id) ? (
              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-4 py-1.5 text-sm font-semibold text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">✅ {t("vocab.learned")} · +{XP_REWARDS.VOCABULARY_WORD_LEARNED} {t("topbar.xp")}</span>
            ) : (
              <button type="button" onClick={() => handleLearn(selectedWord)} className="rounded-xl bg-purple-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-purple-700">
                {t("vocab.mark_learned")} · +{XP_REWARDS.VOCABULARY_WORD_LEARNED} {t("topbar.xp")}
              </button>
            )}
          </div>
        </div>
  ...
  return (
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      <button type="button" onClick={() => navigate("/dashboard")} className="mb-6 flex items-center gap-1 text-sm text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400">
        <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/></svg>
      ...
      <div className="mb-8">
        ...
        <p className="mt-1 text-slate-500 dark:text-slate-400">
          {LANGUAGE_FLAGS[languageCode]} {t("vocabpage.subtitle")} · {t("vocabpage.learnedCount").replace("{learned}", String(learnedCount)).replace("{total}", String(words.length))}
        </p>
      </div>
      ...
      {/* Progress bar */}
      <div className="mb-6">
        <div className="mb-1 flex justify-between text-xs text-slate-500">{t("vocabpage.learnedCount").replace("{learned}", String(learnedCount)).replace("{total}", String(words.length))}</div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((word) => (
          <button type="button" key={word.id} onClick={() => setSelectedWord(word)}
