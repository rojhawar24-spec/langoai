...
// ✅ Coming Soon tekst nu in de taal van de gebruiker
//    EN: "Coming Soon" | NL: "Binnenkort beschikbaar" | FR: "Bientôt disponible"
//    DE: "Demnächst verfügbar" | ES: "Próximamente"
...
export default function AIChat({ open, onClose }: AIChatProps) {
  ...
  // 🔒 FIX (audit #19 — same gap fixed in SettingsPage.tsx, ArenaPage.tsx,
  // and MistakesPage.tsx's modals): role/aria-modal/aria-label were
  // already correct, but nothing moved focus in, trapped it, or closed
  // on Escape.
  useEffect(() => {
    if (!open) return;
  ...
  const features = [
    { icon: "📖", label: t("ai.feature_grammar") },
    { icon: "🌍", label: t("ai.feature_translate") },
    { icon: "💬", label: t("ai.feature_conversation") },
  ];
  ...
  return (
    <>
      ...
      <div
        ...
        aria-modal="true"
        aria-label="AI Tutor"
        className="fixed inset-0 z-50 flex flex-col overflow-hidden bg-white shadow-2xl focus:outline-none dark:bg-slate-950 lg:inset-y-0 lg:left-auto lg:right-0 lg:max-w-lg lg:rounded-l-3xl"
      >
        {/* Header */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">{t("ai.coming_soon")}</p>
              </div>
            </div>
        ...
        <div className="flex flex-1 flex-col items-center justify-center bg-slate-50 px-6 py-12 text-center dark:bg-slate-950">
          ...
          <div className="mt-8 w-full max-w-xs space-y-2">
            {features.map((item) => (
              <div key={item.label} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-400 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-500">
                <span className="text-base opacity-50">{item.icon}</span>
                <span>{item.label}</span>
                <span className="ml-auto text-xs font-medium text-indigo-400 dark:text-indigo-500">
