      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
        ...
        <div className="mx-auto max-w-2xl px-4 py-12 text-center sm:px-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mt-8">
            <button onClick={onBack} className="rounded-xl bg-indigo-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-indigo-700">
              Back to Tests
            </button>
            <button onClick={onDone} className="rounded-xl border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
              Dashboard
            </button>
          </div>
        </div>
  ...
  if (!q) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <button onClick={onBack} className="rounded-xl bg-indigo-600 px-6 py-3 text-sm font-semibold text-white">
          Back to Tests
        </button>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        <div className="flex items-center justify-between mb-6">
          <button onClick={onBack} className="text-sm text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400">
            Back
          </button>
          <span className="text-xs text-slate-400">{current + 1}/{max}</span>
          <button onClick={() => setSubmitted(true)} className="text-sm font-semibold text-indigo-500">
            Submit
          </button>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          ...
          <div className="flex justify-between mt-6">
            ...
            {current + 1 < max ? (
              <button onClick={() => setCurrent(current + 1)} className="rounded-lg bg-indigo-600 px-5 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700">
                Next
              </button>
            ) : (
              <button
    ...
    <div className="min-h-screen bg-[#f8f6f2] dark:bg-[#14141e]">
      ...
      <div className="mx-auto mt-6 max-w-2xl px-4 pb-24">
        <div className={`mb-4 flex items-center gap-2 rounded-xl ${search ? "bg-indigo-50 border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-700" : cls.light} border px-4 py-2.5`}>
          ...
          {search && (
            <button onClick={() => setSearch("")} className="ml-2 text-xs text-indigo-500 hover:text-indigo-600">
              clear
            </button>
          )}

          <span className="ml-auto text-xs text-slate-500">
            {activeTests.length} {search ? "results" : "tests"}
          </span>
        </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[10px] text-slate-400">{test.questionCount} questions</span>
