...
export default async function handler(req, res) {
  ...
  // ── PREMIUM GATE ─────────────────────────────────
  // The frontend route guard is only UX. This server-side check is the real
  // protection for the billed AI endpoint.
  const authHeader = req.headers.authorization || "";
  const sessionToken = authHeader.startsWith("Bearer ") ? authHeader.slice(7) : "";
  if (!sessionToken || !SUPABASE_URL || !SUPABASE_ANON_KEY || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return res.status(401).json({ error: "Premium access required. Please log in and purchase access." });
  }
  ...
  try {
    ...
    const { data: authData, error: authError } = await anonClient.auth.getUser(sessionToken);
    ...
    if (profile.premium !== true || !expiresAt || expiresAt <= Date.now()) {
      return res.status(403).json({ error: "Premium access required. Please contact support to activate Premium access." });
    }
  } catch (err) {
    console.error("Premium verification failed:", err);
    return res.status(503).json({ error: "Premium access could not be verified right now." });
  }
  // ────────────────────────────────────────────────
