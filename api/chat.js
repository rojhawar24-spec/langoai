...
// ✅ Rate limiting: max 15 requests per minute per IP (fail-closed)
...
export default async function handler(req, res) {
  ...
  // Authenticate the caller before using the server-side AI key.
  const authHeader = req.headers.authorization || "";
  const sessionToken = authHeader.startsWith("Bearer ") ? authHeader.slice(7) : "";
  if (!sessionToken || !SUPABASE_URL || !SUPABASE_ANON_KEY) {
    return res.status(401).json({ error: "Login required to use the AI Tutor." });
  }
  ...
  try {
    ...
    return res.status(503).json({ error: "Authentication could not be verified right now." });
  }
