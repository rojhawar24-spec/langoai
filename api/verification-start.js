...
async function rateLimit(key, max, windowSec) {
  try {
    ...
    // FIX: fail-OPEN als Vercel KV niet is geconfigureerd (gratis tier).
    // Rate-limit is dan uit, maar registratie werkt wél.
    // Zet KV aan in Vercel Storage voor echte rate-limiting.
    console.warn("[verification-start] KV unavailable — rate limit skipped");
    return true;
  }
}
